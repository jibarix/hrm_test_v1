...
class DataProcessConfig(BaseModel):
    source_dir: str = "dataset/raw-data/CitySimulator"  # Where you saved the JSON files
    output_dir: str = "data/logistics-routing-1k"
    
def convert_dataset(config: DataProcessConfig):
    # Read the JSON files we generated
    inputs_data = json.load(open(f"{config.source_dir}/city_routing_1000ex_train_all__inputs.json"))
    labels_data = json.load(open(f"{config.source_dir}/city_routing_1000ex_train_all__labels.json"))
    puzzle_identifiers_data = json.load(open(f"{config.source_dir}/city_routing_1000ex_train_all__puzzle_identifiers.json"))
    puzzle_indices_data = json.load(open(f"{config.source_dir}/city_routing_1000ex_train_all__puzzle_indices.json"))
    group_indices_data = json.load(open(f"{config.source_dir}/city_routing_1000ex_train_all__group_indices.json"))
    metadata = json.load(open(f"{config.source_dir}/city_routing_1000ex_train_dataset.json"))
